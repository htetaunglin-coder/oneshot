...
Skill folder: `SKILL.md`, `rules/colocation.md`, `rules/tailwind.md`, `coverage-gaps.md`.

## Method per rule

1. Research: read primary sources only (author blogs, official docs, the skills on skills.sh). Reject AI-written summaries.
...
1. **comments** rule. Sources already read: sergiodxa `comments-meaningful-only`, the user's CLAUDE.md deletion test ("comment survives only if deleting it loses information the code cannot carry"), Kent's Colocation. Expect mostly no-ops; adversarial cases must bait over-commenting and JSDoc on obvious functions.
2. state: zustand vs context vs URL vs server. TkDodo posts already read (Working with Zustand; Zustand and React Context: context is DI, store for state; useCallback posts). wshobson five-row table.
3. file order, magic numbers, conditional-render ladder (early return → sub-component → IIFE last), error and loading, testing, business logic.
4. Move the 1,000-line check to a linter (`max-lines` on ESLint; Biome has none). Add `DECISIONS.md` per rule change.
